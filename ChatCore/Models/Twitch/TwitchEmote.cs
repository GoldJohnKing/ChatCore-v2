...
namespace ChatCore.Models.Twitch
{
	public class TwitchEmote : IChatEmote
	{
		...
		public TwitchEmote(string json)
		{
			...
			{ Uri = uri.Value; }
			if (obj.TryGetKey(nameof(StartIndex), out var startIndex))
			{ StartIndex = startIndex.AsInt; }
			if (obj.TryGetKey(nameof(EndIndex), out var endIndex))
			{ EndIndex = endIndex.AsInt; }
			if (obj.TryGetKey(nameof(IsAnimated), out var isAnimated))
			{ IsAnimated = isAnimated.AsBool; }
			if (obj.TryGetKey(nameof(Bits), out var bits))
			{ Bits = bits.AsInt; }
			if (obj.TryGetKey(nameof(Color), out var color))
			{ Color = color.Value; }
			if (obj.TryGetKey(nameof(Type), out var type))
			{ Type = Enum.TryParse<EmoteType>(type.Value, out var typeEnum) ? typeEnum : EmoteType.SingleImage; }
			if (obj.TryGetKey(nameof(UVs), out var uvs))
			{ UVs = new ImageRect(uvs.Value); }
		}
