...
namespace ChatCore.Models.Twitch
{
	public class TwitchRoomstate
	{
		...
		public TwitchRoomstate(string json)
		{
			...
			if (obj.TryGetKey(nameof(BroadcasterLang), out var broadcasterLang))
			{ BroadcasterLang = broadcasterLang.Value; }
			if (obj.TryGetKey(nameof(RoomId), out var roomId))
			{ RoomId = roomId.Value; }
			if (obj.TryGetKey(nameof(EmoteOnly), out var emoteOnly))
			{ EmoteOnly = emoteOnly.AsBool; }
			if (obj.TryGetKey(nameof(FollowersOnly), out var followersOnly))
			{ FollowersOnly = followersOnly.AsBool; }
			if (obj.TryGetKey(nameof(SubscribersOnly), out var subscribersOnly))
			{ SubscribersOnly = subscribersOnly.AsBool; }
			if (obj.TryGetKey(nameof(R9K), out var r9K))
			{ R9K = r9K.AsBool; }
			if (obj.TryGetKey(nameof(SlowModeInterval), out var slowModeInterval))
			{ SlowModeInterval = slowModeInterval.AsInt; }
			if (obj.TryGetKey(nameof(MinFollowTime), out var minFollowTime))
			{ MinFollowTime = minFollowTime.AsInt; }
		}
